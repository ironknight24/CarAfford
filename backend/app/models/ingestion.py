...
class IngestionRun(Base, TimestampMixin):
    ...
    raw_records = relationship("RawIngestionRecord", back_populates="ingestion_run", cascade="all, delete-orphan")
...
class RawIngestionRecord(Base, TimestampMixin):
    ...
    raw_payload: Mapped[Dict[str, Any]] = mapped_column(JSON().with_variant(JSONB, "postgresql"), nullable=False)
    ...
    validation_errors: Mapped[Optional[List[Dict[str, Any]]]] = mapped_column(JSON().with_variant(JSONB, "postgresql"), default=list, nullable=True)
...
class DataConflictRecord(Base, TimestampMixin):
    ...
    source_a_value: Mapped[Any] = mapped_column(JSON().with_variant(JSONB, "postgresql"), nullable=False)
    source_b_id: Mapped[int] = mapped_column(Integer, ForeignKey("data_sources.id"), nullable=False)
    source_b_value: Mapped[Any] = mapped_column(JSON().with_variant(JSONB, "postgresql"), nullable=False)
    
    detected_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, nullable=False)
    status: Mapped[str] = mapped_column(String(50), default=ConflictStatus.UNRESOLVED.value, nullable=False, index=True)
...
class DataQualityReviewItem(Base, TimestampMixin):
    ...
    raw_record_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("raw_ingestion_records.id", ondelete="SET NULL"), nullable=True, index=True
    )
    entity_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    entity_identifier: Mapped[str] = mapped_column(String(200), nullable=False, index=True)
    field_name: Mapped[str] = mapped_column(String(100), nullable=False)
    
    current_value: Mapped[Optional[Any]] = mapped_column(JSON().with_variant(JSONB, "postgresql"), nullable=True)
    proposed_value: Mapped[Any] = mapped_column(JSON().with_variant(JSONB, "postgresql"), nullable=False)
