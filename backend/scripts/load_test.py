...
async def make_request(client: httpx.AsyncClient, workload: Dict[str, Any], worker_id: int) -> Tuple[float, int, bool]:
    t0 = time.perf_counter()
    headers = {"X-Forwarded-For": f"10.0.{worker_id // 250}.{worker_id % 250 + 1}"}
    try:
...
async def run_concurrency_batch(workload: Dict[str, Any], concurrency: int, total_requests: int) -> Dict[str, Any]:
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        ...
        async def worker(w_id: int):
            async with sem:
                return await make_request(client, workload, w_id)
    ...
    rps = total_requests / max(total_time, 0.001)
...
async def main():
    print("=" * 80, flush=True)
    print("🏎️  CarAfford Performance & Load Testing Benchmark", flush=True)
    print(f"Target: {API_BASE_URL}", flush=True)
    print("=" * 80, flush=True)

    concurrency_levels = [10, 25, 50]

    for workload in WORKLOADS:
        print(f"\n📊 Profiling Endpoint: [{workload['name']}]", flush=True)
        print("-" * 80, flush=True)
        print(f"{'Concurrency':<12} | {'Reqs':<6} | {'RPS':<8} | {'p50 (ms)':<10} | {'p95 (ms)':<10} | {'p99 (ms)':<10} | {'Errors':<8}", flush=True)
        print("-" * 80, flush=True)
    ...
    print("\n" + "=" * 80, flush=True)
    print("✅ Performance benchmark completed successfully!", flush=True)
    print("=" * 80, flush=True)
