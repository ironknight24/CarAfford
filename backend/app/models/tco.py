...
class FuelPrice(Base, TimestampMixin):
    ...
    observed_date: Mapped[datetime] = mapped_column(
...
class ElectricityTariff(Base, TimestampMixin):
    ...
    tariff_type: Mapped[str] = mapped_column(
        String(50), default="DOMESTIC_SLAB", nullable=False
    )  # DOMESTIC_SLAB, EV_SPECIAL_TARIFF, COMMERCIAL
    state_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("states.id"), nullable=True, index=True
    )
    state_code: Mapped[Optional[str]] = mapped_column(String(10), nullable=True, index=True)
...
class MaintenanceCostBenchmark(Base, TimestampMixin):
    ...
    powertrain: Mapped[str] = mapped_column(
        String(50), index=True, nullable=False
...
class DepreciationBenchmark(Base, TimestampMixin):
    ...
    methodology: Mapped[str] = mapped_column(
        String(100), default="EMPIRICAL_MARKET_RESALE", nullable=False
    )
    effective_from: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    effective_to: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
