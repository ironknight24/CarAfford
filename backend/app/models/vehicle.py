...
class CarModel(Base, TimestampMixin, AuditableMixin):
    ...
    )  # A-Segment, B-Segment, Compact SUV, etc.
    active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False, index=True)
    ...
    media: Mapped[List["VehicleMedia"]] = relationship(
        ...
        cascade="all, delete-orphan",
        foreign_keys="VehicleMedia.model_id",
    )
...
class Variant(Base, TimestampMixin, AuditableMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "engine_cc IS NULL OR engine_cc >= 0", name="chk_variants_engine_cc_positive"
        ),
        CheckConstraint(
            "battery_capacity_kwh IS NULL OR battery_capacity_kwh >= 0",
            name="chk_variants_battery_kwh_positive",
        ),
    )
    ...
    drivetrain: Mapped[Optional[str]] = mapped_column(
        String(20), default="FWD", nullable=True, index=True
    )  # FWD, RWD, AWD, 4WD
    ...
    mileage_claimed: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(6, 2), nullable=True
    )  # ARAI km/l, km/kg, or km/kWh
    ...
    prices: Mapped[List["VehiclePrice"]] = relationship(
        ...
        cascade="all, delete-orphan",
        order_by="VehiclePrice.effective_from.desc()",
    )
    media: Mapped[List["VehicleMedia"]] = relationship(
        "VehicleMedia",
        back_populates="variant",
        cascade="all, delete-orphan",
        foreign_keys="VehicleMedia.variant_id",
    )
...
class VariantSpecification(Base, TimestampMixin, AuditableMixin):
    ...
    arai_mileage_kmpl: Mapped[Decimal] = mapped_column(
...
class VehicleMedia(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "variant_id IS NOT NULL OR model_id IS NOT NULL",
            name="chk_vehicle_media_target_not_null",
        ),
    )
    ...
    media_type: Mapped[str] = mapped_column(
        String(50), default="IMAGE_EXTERIOR", nullable=False, index=True
    )
    ...
    variant: Mapped[Optional["Variant"]] = relationship(
        "Variant", back_populates="media", foreign_keys=[variant_id]
    )
    model: Mapped[Optional["CarModel"]] = relationship(
        "CarModel", back_populates="media", foreign_keys=[model_id]
    )
