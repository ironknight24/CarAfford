        ...
        if price_val is None:
            ...
            try:
                ...
                if dec_price <= Decimal("0.00"):
                    ...
                    errors.append(
                        f"Price ₹{dec_price:,.2f} is outside reasonable automotive bounds (₹1L - ₹5Cr)"
                    )
            except Exception:
            ...
            if isinstance(eff_to, str):
                try:
                    ...
                    if isinstance(eff_from, datetime) and eff_from >= to_dt:
                        errors.append(
                            f"effective_from ({eff_from}) must be strictly earlier than effective_to ({to_dt})"
                        )
                except Exception:
                ...
                if eff_from >= eff_to:
                    errors.append(
                        f"effective_from ({eff_from}) must be strictly earlier than effective_to ({eff_to})"
                    )
